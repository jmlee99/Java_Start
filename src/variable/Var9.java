...
public class Var9 {
    ...
    * 규칙은 필수이다. 규칙을 지키지 않으면 오류가 난다.
    * 관례는 필수는 아니지만 전세계 개발자가 해당 관례를 따르기 때문에 사실상 규칙이라고 생각*/

    /*규칙
    * 1. 변수 이름은 숫자로 시작할 수 없다 (Ex: 1num, 1st)
    * - 그러나 숫자를 이름에 포함하는 것은 가능하다.(Ex: myVar1, num1)
    * 2. 이름에는 공백이 들어갈 수 없다.
    * 3. 자바의 예약어를 변수 이름으로 사용할 수 없다. (Ex: int, class, public)
    * 4. 변수 이름에는 영문자(a-z, A-Z), 숫자(0-9), 달러 기호($) 또는 밑줄(_)만 사용할 수 있다.*/

    /*관례
    * 1. 소문자로 시작하는 낙타 표기법
    * - 변수 이름은 소문자로 시작하는 것이 일반적이다. 여러 단어로 이루어진 변수 이름의 경우, 첫 번째 단어는 소문자로 시작하고
    * 그 이후의 각 단어는 대문자로 시작하는 낙타 표기법을 사용한다. (Ex : orderDetail, myAccount)*/

    /*자바 언어의 관례 한번에 정리
    * # 클래스는 대문자로 시작, 나머지는 소문자로 시작 #
    * 1. 자바에서 클래스 이름의 첫 글자는 대문자로 시작한다. 그리고 나머지는 모두 첫 글자를 소문자로 시작한다.
